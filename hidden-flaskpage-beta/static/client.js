...
function pushMessage(args) {
	...
	// Trip
	if (args.trip) {
		// Only show trip in nick's title unless it's a emote message
		if (args.cmd === "chat") {
			nickEl.setAttribute("title","Trip: " + args.trip);
		} else {
			var tripEl = document.createElement('addr');
	...
	// Nick
	if (args.nick) {
		if (args.color == "colorful") {
			nickEl.classList.add("colorful");
		} else if (args.nick === "ee") {
			nickEl.classList.add("eecolor");
		} else if (args.level == "mod") {
			nickEl.classList.add("modcolor");
		} else if (args.color && /(^[0-9A-F]{6}$)|(^[0-9A-F]{3}$)/i.test(args.color)) {
			nickEl.setAttribute('style', 'color:#' + args.color + ' !important');
		}
		nickEl.textContent = args.nick;
	...
	if (args.statu) {
		...
		signEl.textContent = String.fromCodePoint(11088);
		nickSpanEl.appendChild(signEl);
	}
	...
	if ((lastischat && lasttalk != args.nick) || lastischat != true) {
		messageEl.appendChild(nickSpanEl);
	}
